...
class Dataprocessing
{
     ...
     *
     * Метод создает новую запись в таблице БД (По представленной ранее схеме БД в задании)
     */
     ...
     *
     * Метод проверяет наличие введенных оператором данных
     *
     * @return boolean функция возвращает информацию о том, все ли необходимые поля были заполнены оператором
     */
    public function inputValidation($eventID, $eventDate, $ticketAdultPrice, $ticketAdultQuantity, $ticketKidPrice, $ticketKidQuantity)
    {
    ...
    public function addBarcode($barcode)
    {
        ...
        //Добавляем запись в таблицу с пользователями
        $stmt = $dbh->prepare("INSERT INTO `Barcode` 
				(`id`, `Barcode`) values( , :barcode)")
